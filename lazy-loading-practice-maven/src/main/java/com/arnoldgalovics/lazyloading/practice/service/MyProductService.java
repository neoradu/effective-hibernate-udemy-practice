...
public class MyProductService {
    private Logger logger = LoggerFactory.getLogger(MyProductService.class);
    ...
    @Transactional
    public int countReviewsForProduct(int productId) {
        Product product = entityManager.find(Product.class, productId);
        logger.debug("After Product product = entityManager.find(Product.class, productId);");
        return product.getReviews().size();
    }
    
    @Transactional
    public List<ProductReview> getReviewsByProductHibernateAPI(int productId) {
        Product product = entityManager.find(Product.class, productId);
    ...
    public int countAvailableProducts() {
        int productCnt = 0;
        TypedQuery<Product> productQuery = entityManager.createQuery("FROM Product products", Product.class);
        List<Product> products = productQuery.getResultList();
        for (Product p : products) {
            productCnt += p.getReviews().size();
        }
